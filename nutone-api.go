...
func dbInsertAttackerUIDs(k KillEvent) error {
	dbMutex.Lock()
	defer dbMutex.Unlock()
...
func dbInsertVictimUIDs(k KillEvent) error {
	dbMutex.Lock()
	defer dbMutex.Unlock()
...
func dataHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = dbInsertAttackerUIDs(killEvent)
	if err != nil {
		log.Print(err)
		http.Error(w, "database error", http.StatusInternalServerError)
		return
	...
	err = dbInsertVictimUIDs(killEvent)
	if err != nil {
		log.Print(err)
		http.Error(w, "database error", http.StatusInternalServerError)
		return
