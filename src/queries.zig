...
pub const Get = struct {
        ...
        pub fn WeaponData(allocator: std.mem.Allocator, filters: Filters) ![:0]u8 {
            if (filters.weapon != null) {
                const query = std.mem.joinZ(allocator, " ", &.{
                    "select count(1) as kills, avg(distance) as avg_distance from kill_data where attacker_uid = ?3 and attacker_uid <> victim_uid and cause_of_death = ?2",
                    if (filters.server != null) "and server_id = ?1" else "",
                ...
                const query = std.mem.joinZ(allocator, " ", &.{
                    "select cause_of_death, count(1) as kills, avg(distance) as avg_distance from kill_data where attacker_uid = ?3 and attacker_uid <> victim_uid",
                    if (filters.server != null) "and server_id = ?1" else "",
                    "group by cause_of_death",
                });
                return query;
    ...
    pub const Players = struct {
        pub fn Data(allocator: std.mem.Allocator, filters: Filters) ![:0]u8 {
            if (filters.weapon != null) {
                const query = try std.mem.joinZ(allocator, " ", &.{
                    "select players.uid as id, players.name as name, (select count(1) from kill_data where players.uid = kill_data.attacker_uid and kill_data.victim_uid <> kill_data.attacker_uid and kill_data.cause_of_death = ?2",
                    if (filters.server != null) "and kill_data.server_id = ?1" else "",
                    ") as kills, (select avg(kill_data.distance) from kill_data where players.uid = kill_data.attacker_uid and kill_data.attacker_uid <> kill_data.victim_uid and kill_data.cause_of_death = ?2",
                    if (filters.server != null) "and kill_data.server_id = ?1" else "",
        ...
        pub fn Count(allocator: std.mem.Allocator, filters: Filters) ![:0]u8 {
            if (filters.weapon != null) {
                const query = try std.mem.joinZ(allocator, " ", &.{
                    "select count(1) from (select distinct attacker_uid from kill_data where cause_of_death = ?2",
                    if (filters.server != null) "and server_id = ?1" else "",
                    ")",
                });
                return query;
