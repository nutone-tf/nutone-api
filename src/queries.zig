...
const getAllPlayerDataForWeapon = "select players.uid as id, players.name as name, (select count(1) from kill_data where players.uid = kill_data.attacker_uid and kill_data.victim_uid <> kill_data.attacker_uid and kill_data.attacker_weapon = ?1) as kills, (select avg(kill_data.distance) from kill_data where players.uid = kill_data.attacker_uid and kill_data.attacker_uid <> kill_data.victim_uid and kill_data.attacker_weapon = ?1) as avg_distance from players where name = (select players.name from players where players.uid = id order by timestamp desc limit 1) order by kills desc limit ?2 offset ?3";
